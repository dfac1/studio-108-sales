...
export async function handleSalesDialog(input: SalesDialogInput): Promise<SalesDialogResult> {
  ...
  if (!state.customerName) {
    ...
    // Если клиент в первом сообщении уже спросил про студию — ответим коротко
    // и сразу спросим имя в той же реплике. Раньше игнорировали вопрос (просто greeting).
    if (isStudioInfoQuestion(lower)) {
      return ask(
        "ask_name",
        "Здравствуйте! Это Studio 108 — танцевальная студия с тремя филиалами: на Развилке, у озера и возле первой школы. Групповые занятия для детей и взрослых, разные направления. Подскажите, как к вам обращаться?",
        state
      );
    }
...
// «Расскажите» намеренно НЕ включён сюда — это специфический информационный запрос
// («расскажите про студию», «расскажите про йогу»), который должен ловиться
// isStudioInfoQuestion / clarify-about-direction, а не интерпретироваться как «подбирай сам».
// Иначе фраза «расскажите про студию» силиково выбирает направление и едет к филиалу,
// игнорируя сам вопрос клиента.
const CLIENT_GIVES_INITIATIVE = /(не\s+знаю|не\s+определил|без\s+понятия|без\s+разниц|посоветуй|подскажи|что\s+посовет|подберит|перечислит|какие|что\s+у\s+вас|что\s+есть|популярн|хит|что\s+сейчас\s+попул|что\s+нравится\s+детям|вы\s+посовет|подберите)/i;
...
function isStudioInfoQuestion(text: string): boolean {
  ...
  // КРИТИЧНО: \b в JS не работает с кириллицей — после Cyrillic-буквы [а-яё] и пробела
  // оба символа считаются \W, и word-boundary не срабатывает. Убрали \b — non-greedy
  // [\s\S]{0,40}? сам по себе достаточен.
  if (/(?:расскаж[а-яё]+|поведай[а-яё]*|опиши[а-яё]*|поясн[а-яё]*)[\s\S]{0,40}?(?:про|о|об)\s+(?:вашу?\s+|вашей\s+|вашем\s+|свою\s+|своей\s+|своей\s+)?(?:студи|школ|компани|зал|клуб|вас|себ)/i.test(t)) {
    return true;
  }
